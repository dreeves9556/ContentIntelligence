...
- "cta": the closing call-to-action. For Reels: spoken at the end. For Carousels: a short pasteable closer (may also be repeated in the caption). For Static: leave this field EMPTY ("") — the CTA belongs in the caption, not on the image.
- "caption": the social media caption pasted BELOW the post in the feed. This is where the longer explanation, story, context, and engagement hook go for Static and Carousel posts. For Static posts, the caption MUST also include the call-to-action (e.g., "Reply CHART for the guide", "Link in bio to book"). Do NOT include any hashtags. Do NOT repeat the on-image text verbatim — expand on it or give the context the image can't carry.
...
STATIC POST RULE (critical): A Static post is a single image with short text overlaid on it. The "body" is what goes ON the image (under 300 characters). The "caption" is what goes BELOW the image in the feed, and it MUST include the call-to-action. The "cta" field must be EMPTY ("") for Static posts — do not put the CTA on the image. Do NOT write a long essay as the body. If you have a lot to say, put it in the caption.
